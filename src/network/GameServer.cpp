...
GameServer::GameServer(ECS::Registry& ecs, uint16_t port, const std::string& protocol)
    ...
    , _port(port)
    , _running(false) {
    ProtocolManager proto;  // config/protocol.json
    _server = std::make_unique<Server>(protocol, port, proto);
}
...
void GameServer::update(float delta_time) {
    ...
    if (_protocol_type == SocketType::UDP) {
...
bool GameServer::sendTo(const Address& client, const std::vector<uint8_t>& data) {
...
void GameServer::broadcastToAll(const std::vector<uint8_t>& data, const Address* exclude) {
...
void GameServer::setPacketCallback(PacketCallback callback) {
...
std::vector<Address> GameServer::getConnectedClients() const {
...
void GameServer::updateUDP(float delta_time) {
    try {
        std::vector<Address> senders = _server->udpReceive(100, 100);
...
void GameServer::updateTCP(float delta_time) {
    try {
        ...
        for (int fd : ready_fds) {
            ...
            Address* addr_ptr = nullptr;
            for (const auto& [addr, client_fd] : _address_to_fd) {
                if (client_fd == fd) {
                    addr_ptr = const_cast<Address*>(&addr);
                    break;
                }
            }
            if (!addr_ptr)
...
void GameServer::checkClientTimeouts() {
    ...
    for (const Address& addr : to_remove) {
