...
static void load_player(ECS::Registry& reg, const sf::Vector2u& pos)
{
    ...
    reg.addComponent(MAP_ENTITY_PLAYER, te::Position2(pos.x * SQUARE_WIDTH, pos.y * SQUARE_HEIGHT));
    ...
    reg.addComponent(MAP_ENTITY_PLAYER, te::Interactive(-15.0f, -15.0f, 60.0f, 60.0f));
}
...
static void create_square(ECS::Registry& reg, const ECS::Entity& entity_id, char type, const sf::Vector2u& pos)
{
    if (type == MAP_WALL) {
        ...
        reg.addComponent(entity_id, te::Position2(pos.x * SQUARE_WIDTH, pos.y * SQUARE_HEIGHT));
        reg.addComponent(entity_id, te::Sprite(TEXTURE_MAP.at(MAP_WALL)));
        reg.addComponent(entity_id, te::Hitbox(0.0f, 0.0f, SQUARE_WIDTH, SQUARE_WIDTH));
    } else if (type == MAP_DOOR) {
        reg.addComponent(entity_id, te::Drawable());
        reg.addComponent(entity_id, te::Position2(pos.x * SQUARE_WIDTH, pos.y * SQUARE_HEIGHT));
        reg.addComponent(entity_id, te::Sprite(TEXTURE_MAP.at(MAP_DOOR)));
        reg.addComponent(entity_id, te::Hitbox(0.0f, 0.0f, SQUARE_WIDTH, SQUARE_WIDTH));
        reg.addComponent(entity_id, te::Interactive(-15.0f, -15.0f, 60.0f, 60.0f, [](ECS::Registry& reg){
