    ...
    _components["sprite"] = [](ECS::Registry& reg, const ECS::Entity& e,
        ...
        try {
            if (json.find("texture") != json.end()) {
                sf::Texture texture = std::any_cast<sf::Texture>(json.at("texture"));
                reg.addComponent(e, te::Sprite(std::move(texture)));
                return;
            }
            std::filesystem::path path(std::any_cast<std::string>(json.at("path")));
            sf::Texture texture(path);
            if (json.find("size") != json.end()) {
                sf::Vector2f size = std::any_cast<sf::Vector2f>(json.at("size"));
