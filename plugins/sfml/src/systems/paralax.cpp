...
void paralax_sys(ECS::Registry &reg) {
    ...
    auto& drawables = reg.getComponents<sfml::Drawable>();
    ...
    for (auto&& [win] : ECS::Zipper(windows)) {
        for (auto&& [para, sp, draw, pos] :
            ...
            auto size = sp.getTextureRect().size;
            size.x *= sp.getScale().x;
            size.y *= sp.getScale().y;
            if (para.reset.x > pos.x + size.x) {
                pos.x = para.reset.x;
            }
            for (std::size_t i = 0; i < para.iterations; ++i) {
                sp.setPosition({pos.x + size.x * i, pos.y});
                win.draw(sp);
            }
        }
    }
