...
#define CAST_M(a, b) std::chrono::duration_cast<std::chrono::microseconds>(a - b).count()
...
struct Timestamp {
 public:
    Timestamp(float delay, bool active = true)
        : delay((size_t)SEC_TO_MICRO(delay)), ref(NOW), cur(ref), active(active) {}
